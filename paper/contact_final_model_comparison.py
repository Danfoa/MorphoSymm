...
if __name__ == "__main__":
    ...
    filter_hps = [
        "train_ratio=0.85",  #'finetuned=True',
        #'model=ECNN',
        # 'augment=True',
        #'model=EMLP',
    ]
    unique_hps = {"finetuned": {True, False}, "train_ratio": {0.85}, "dataset.augment": {True, False}}
    ...
    for leg_name in ["LF", "LH", "RF", "RH", "legs_avg"]:
        state_metrics = [
            c for c in df.columns if f"test_{leg_name}/f1" in c
        ]  # + ['test_contact_state/acc', 'Model Type', 'Hyper params']
        state_metrics += [c for c in df.columns if f"test_{leg_name}/acc" in c] + [
            "test_contact_state/acc",
            "Model Type",
            "Hyper params",
        ]
        ...
        df_state.columns = df_state.columns.str.replace("Leg-legs_avg", "Legs-Avg")
        df_state.columns = df_state.columns.str.replace("test_contact_state/", "State\n")
        df_state.columns = df_state.columns.str.replace("f1", "F1")
        df_state.columns = df_state.columns.str.replace("acc", "Acc")
        metrics = list(df_state.columns)
        df_state.loc[:, "leg"] = leg_name
        df_state = df_state.melt(
            id_vars=["leg", "Model Type", "Hyper params"],
            value_vars=metrics,
            var_name="metric",
    ...
    sns.barplot(
        ...
        x="metric",
        y="Metric Score",
        order=sorted(np.unique(df_legs_states["metric"])),
        hue="Model Type",
        hue_order=model_types,  # kind="bar",
        errwidth=1.5,
        linewidth=0,
        capsize=0.025,
        # style='Model Type', style_order=model_types,
    ...
    # ax.set_ylabel('')
    plt.tight_layout()
    plt.savefig(out_path / "legs_contact_state_metrics.png")
    plt.show()
