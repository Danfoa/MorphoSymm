...
def get_model(cfg: DictConfig, in_field_type=None, out_field_type=None):
    ...
    if "ecnn" in cfg.model_type.lower():
        ...
        model = ContactECNN(
            ...
            inv_dim_scale=cfg.inv_dims_scale,
            bias=cfg.bias,
        )
...
def get_datasets(cfg, device, root_path):
    """Get datasets and dataloaders for the experiment."""
    if cfg.dataset.name == "contact":
        ...
        train_dataset = UmichContactDataset(
            data_name="train.npy",
            robot_cfg=cfg.robot,
            label_name="train_label.npy",
        ...
        val_dataset = UmichContactDataset(
            data_name="val.npy",
            robot_cfg=cfg.robot,
            label_name="val_label.npy",
            train_ratio=cfg.dataset.train_ratio,
            augment=False,
            use_class_imbalance_w=False,
            window_size=cfg.dataset.window_size,
        ...
        test_dataset = UmichContactDataset(
            data_name="test.npy",
            robot_cfg=cfg.robot,
        ...
        if cfg.dataset.balanced_classes:
            ...
            class_freqs = torch.maximum(
                class_freqs, torch.ones_like(class_freqs) * (1 / train_dataset.n_contact_states)
            )
        ...
        train_dataset = COMMomentum(
            robot_cfg=cfg.robot,
            type="train",
            samples=cfg.dataset.samples,
            train_ratio=cfg.dataset.train_ratio,
            angular_momentum=cfg.dataset.angular_momentum,
            standarizer=cfg.dataset.standarize,
            augment=cfg.dataset.augment,
            data_path=data_path,
            dtype=torch.float32,
            device=device,
...
def get_ckpt_storage_path(log_path, use_volatile=True):  # noqa: D103
    if not use_volatile:
        return log_path
    try:
        ...
        log.warning(
            f"Volatile storage {e} not found, default to current log dir for model checkpointing: "
...
def main(cfg: DictConfig):  # noqa: D103
    ...
    if not training_done:
        ...
        # Prepare model
        model = get_model(cfg.model, in_field_type=train_dataset.in_type, out_field_type=train_dataset.out_type)
        log.info(model)
