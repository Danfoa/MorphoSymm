...
from robot_kinematic_symmetries import JointSpaceSymmetry, is_equivariant_invariant
...
if __name__ == "__main__":
    ...
    Permutations = [list(reversed(idxOut))]
    ...
    for gout in Permutations:
        ...
        if not is_canonical_permutation(Pout):
            continue
        if (Din % 2 == 0 or Dout % 2 == 0) and np.any(np.asarray(gout) - idxOut == 0):
            continue

        h_in = C2.oneline2matrix(oneline_notation=gin)
        Gin = C2(h_in)
        reflections_gout = np.ones_like(idxOut)
        reflection_ix = np.random.choice(idxOut)
        reflections_gout[[reflection_ix, gout[reflection_ix]]] *= -1
        h_out = C2.oneline2matrix(oneline_notation=gout, reflexions=reflections_gout)
        Gout = C2(h_out)
        ...
        Q_sub = Vin_out.equivariant_basis()
