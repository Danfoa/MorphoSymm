...
if __name__ == '__main__':
    ...
    while True:
        ...
        robot.reset_state(q=np.concatenate([base_pos + traj_q[t_idx, :3], base_ori, traj_q_js]),
                          v=np.zeros(robot.nv))
        ...
        for n, iso_robot, iso_id in zip(range(len(iso_robots)), iso_robots, traj_q_iso.keys()):
            ...
            iso_robot.reset_state(q=np.concatenate([base_positions[n] + traj_q_iso[iso_id][t_idx, :3],
                                                    base_ori_iso_i,
                                                    traj_q_js_iso_i]),
                                  v=np.zeros(robot.nv))
