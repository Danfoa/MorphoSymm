...
class A1(PinBulletWrapper):
    ...
    def __init__(self, resources: pathlib.Path, control_mode=ControlMode('torque'), power_coeff=1.0,
        ...
        super(A1, self).__init__(resources=resources, recontrol_mode=control_mode, useFixedBase=useFixedBase,
    ...
    def load_pinocchio_robot(self, reference_robot: Optional['PinBulletWrapper'] = None) -> RobotWrapper:
        ...
        pin_robot = pin_load_robot_description("a1_description", root_joint=JointModelFreeFlyer())
    ...
    def load_bullet_robot(self, base_pos=None, base_ori=None) -> int:
        ...
        robot_id = pb_load_robot_description("a1_description", basePosition=base_pos,
    ...
    def configure_bullet_simulation(self, bullet_client: BulletClient, world, base_pos=None, base_ori=None):
        super(A1, self).configure_bullet_simulation(bullet_client, world, base_pos, base_ori)
        # Initial config
    ...
    def endeff_names(self) -> List:
        return ['FL_foot_fixed', 'FR_foot_fixed', 'RL_foot_fixed', 'RR_foot_fixed']

    def get_init_config(self, random=False):
        f_leg_pos = np.array([0.0, 0.4, -1.2])
        h_leg_pos = np.array([0.0, 0.4, -1.2])
        ...
        base_ori = [0, 0, 0, 1]
        if random:
            pitch = np.random.uniform(low=-np.deg2rad(35), high=np.deg2rad(35))
            roll = np.random.uniform(low=-np.deg2rad(35), high=np.deg2rad(35))
            yaw = np.random.uniform(low=-np.deg2rad(35), high=np.deg2rad(35))
